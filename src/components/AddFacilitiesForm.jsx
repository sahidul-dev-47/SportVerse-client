              ...
              {step === 1 && (
                <motion.div key="s1" variants={slide} initial="hidden" animate="visible" exit="exit" className="space-y-5">
                  ...
                  <Field label="Facility Name">
                    <input className={inputCls} placeholder="e.g. Green Turf Arena" value={form.facilityName} onChange={patch("facilityName")} />
                  </Field>
                  ...
                  <Field label="Location">
                    <div className="relative">
                      ...
                      <input className={`${inputCls} pl-10`} placeholder="e.g. Gulshan, Dhaka" value={form.location} onChange={patch("location")} />
                    </div>
                  </Field>
                </motion.div>
              )}
                ...
                <motion.div key="s2" variants={slide} initial="hidden" animate="visible" exit="exit" className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="Price Per Hour (৳)">
                      <input className={inputCls} type="number" min="0" placeholder="e.g. 1500" value={form.pricePerHour} onChange={patch("pricePerHour")} />
                    </Field>
                    <Field label="Capacity (persons)">
                      <input className={inputCls} type="number" min="1" placeholder="e.g. 22" value={form.capacity} onChange={patch("capacity")} />
                    </Field>
                  </div>

                  {/* ── শুধু Image URL input ── */}
                  ...
                  <Field label="Description">
                    <textarea className={`${inputCls} resize-none`} rows={4} placeholder="Surface type, amenities, parking, changing rooms…" value={form.description} onChange={patch("description")} />
                  </Field>
                </motion.div>
              )}
              ...
              {step === 4 && (
                <motion.div key="s4" variants={slide} initial="hidden" animate="visible" exit="exit" className="space-y-5">
                  ...
                  <Field label="Owner Email · auto-filled from your account">
                    <input className={`${inputCls} opacity-50 cursor-not-allowed`} value={form.ownerEmail} readOnly />
                  </Field>
                </motion.div>
              )}
          ...
          <div className="border-t border-white/5 px-6 md:px-8 py-4 flex items-center justify-between gap-4">
            ...
            {step < 4 ? (
              ...
              <motion.button
                ...
                className="px-6 py-2.5 bg-emerald-400 text-[#0a0f1a] text-sm font-black rounded-xl hover:bg-emerald-300 transition-colors"
              >
                🚀 Go Live
              </motion.button>
            )}
...
function StepHeader({ title, sub }) {
  return (
    <div className="mb-1">
      ...
      <p className="text-white/38 text-sm mt-0.5">{sub}</p>
    </div>
  );
}
