...
import {
  MdSportsSoccer, MdLocationOn, MdAttachMoney, MdPeople,
  MdImage, MdEmail, MdDescription, MdCheckCircle,
  MdArrowBack, MdArrowForward, MdRocketLaunch,
} from "react-icons/md";
...
const toastStyle = { background: "#111827", color: "#fff", border: "1px solid rgba(255,255,255,0.08)" };
...
function ImagePreview({ url }) {
  ...
  if (url !== prevUrl) { setPrevUrl(url); setLoaded(false); setErrored(false); }
  if (!url) return null;
...
export default function AddFacilitiesForm() {
  ...
  useEffect(() => {
    authClient.getSession().then(({ data: session }) => {
      ...
      if (email) {
        setForm((f) => ({ ...f, ownerEmail: email.trim().toLowerCase() }));
      }
    }).catch(() => {});
  }, []);
  ...
  const handleNext = () => { if (step < 4 && canNext()) setStep((s) => s + 1); };
...
const handleGoLive = async () => {
  ...
  const resetForm = () => {
    ...
    setForm((prev) => ({
      ...
      image: "", pricePerHour: "", capacity: "", description: "",
    ...
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
        ...
        >
          ...
          <h2 className="text-3xl font-black text-white mt-3 mb-3 tracking-tight">Facility is Live! 🎉</h2>
          <p className="text-gray-400 text-sm leading-relaxed mb-2">
            <span className="text-white font-bold">{form.facilityName}</span> 
          </p>
          <p className="text-gray-600 text-xs mb-8">
            Listed under <span className="text-gray-300">{form.ownerEmail}</span>
          </p>
          <button
            type="button"
            onClick={resetForm}
      ...
      <div className="relative w-full max-w-2xl">
        ...
        <motion.div initial={{ opacity: 0, y: -18 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-600/15 border border-blue-500/25 text-blue-400 text-[10px] font-bold tracking-widest uppercase mb-5">
            <IoFlash size={11} /> Facility Portal
          </span>
          <h1
          ...
          <p className="text-gray-500 text-sm mt-2">Get your sports venue live on SportVerse in minutes.</p>
          {form.ownerEmail && (
            <p className="text-xs text-gray-600 mt-2">
              Listing as <span className="text-blue-400 font-semibold">{form.ownerEmail}</span>
            </p>
          )}
        </motion.div>
            ...
            <div key={s.id} className="flex items-center flex-1">
              <div className="flex flex-col items-center gap-2">
                <motion.button
                  type="button"
                  animate={{
                    backgroundColor: step > s.id ? "#2563eb" : step === s.id ? "rgba(37,99,235,0.15)" : "rgba(255,255,255,0.04)",
                    borderColor: step >= s.id ? "#3b82f6" : "rgba(255,255,255,0.1)",
                  }}
                ...
                >
                  {step > s.id ? <MdCheckCircle size={20} className="text-white" /> : <span>{s.icon}</span>}
                </motion.button>
        ...
        <div className="relative rounded-2xl bg-white/[0.03] border border-white/10 overflow-hidden">
              ...
              {step === 3 && (
                <motion.div key="s3" variants={slide} initial="hidden" animate="visible" exit="exit" className="flex flex-col gap-5">
                  ...
                  {selectedSlots.length > 0 && (
                    <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-blue-400/70 text-xs text-center">
                ...
                <motion.div key="s4" variants={slide} initial="hidden" animate="visible" exit="exit" className="flex flex-col gap-5">
                  ...
                  <div className="rounded-xl border border-white/8 bg-white/[0.02] divide-y divide-white/5 overflow-hidden">
                    ...
                    <div className="px-4 py-3">
                      <p className="text-xs text-gray-500 mb-2.5 flex items-center gap-2"><span>🕐</span> Time Slots</p>
                      <div className="flex flex-wrap gap-1.5">
                        {selectedSlots.map((s) => (
                          <span key={s} className="text-[10px] bg-blue-600/12 border border-blue-500/25 text-blue-300 rounded-full px-3 py-1">{s}</span>
                        ))}
                      </div>
                    </div>
                    <div className="px-4 py-3">
          ...
          <div className="border-t border-white/5 px-6 md:px-8 py-4 flex items-center justify-between gap-4">
            ...
            <div className="flex gap-1.5 items-center">
              {STEPS.map((s) => (
                <div key={s.id} className={`rounded-full h-1.5 transition-all duration-300 ${
                  step === s.id ? "w-6 bg-blue-500" : step > s.id ? "w-1.5 bg-blue-500/50" : "w-1.5 bg-white/15"
                }`} />
              ))}
            </div>
