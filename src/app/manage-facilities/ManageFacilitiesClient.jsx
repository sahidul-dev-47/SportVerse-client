          ...
          <div className="flex flex-col gap-5 p-6 overflow-y-auto flex-1">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Facility Name" icon={MdSportsSoccer}>
                <div className="relative">
                  ...
                  <input type="text" className={`${inp} pl-10`} value={form.facilityName} onChange={patch("facilityName")} />
                </div>
              </Field>
              <Field label="Facility Type" icon={IoFlash}>
                <select value={form.facilityType} onChange={patch("facilityType")} className={`${inp} appearance-none`}>
            ...
            <Field label="Location" icon={MdLocationOn}>
              <div className="relative">
                ...
                <input type="text" className={`${inp} pl-10`} value={form.location} onChange={patch("location")} />
              </div>
            </Field>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Price Per Hour ($)" icon={MdAttachMoney}>
                <div className="relative">
                  ...
                  <input type="number" min="0" className={`${inp} pl-9`} value={form.pricePerHour} onChange={patch("pricePerHour")} />
                </div>
              </Field>
              <Field label="Capacity (persons)" icon={MdPeople}>
                <div className="relative">
                  ...
                  <input type="number" min="1" className={`${inp} pl-9`} value={form.capacity} onChange={patch("capacity")} />
                </div>
              </Field>
            </div>
            ...
            <Field label="Image URL" icon={MdImage}>
              <div className="relative">
                ...
                <input type="url" className={`${inp} pl-10`} placeholder="https://..." value={form.image} onChange={patch("image")} />
              </div>
            ...
            <Field label="Description" icon={MdDescription}>
              <div className="relative">
                ...
                <textarea rows={3} className={`${inp} pl-10 resize-none`} value={form.description} onChange={patch("description")} />
              </div>
            </Field>
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl border text-xs font-medium text-left transition-all duration-200 ${
                        on ? "bg-blue-600/15 border-blue-500/40 text-blue-300" : "bg-white/3 border-white/8 text-gray-500 hover:border-white/20 hover:text-gray-300"
                      }`}
                    >
...
function DeleteModal({ facility, ownerEmail, onClose, onDeleted }) {
  ...
  return (
    <>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 bg-black/75 backdrop-blur-sm z-40" />
      <motion.div initial={{ opacity: 0, scale: 0.9, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 10 }} className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={(e) => e.stopPropagation()}>
        <div className="relative w-full max-w-sm rounded-2xl bg-gray-900 border border-red-500/20 overflow-hidden shadow-2xl shadow-black/60">
          ...
          <div className="p-7 flex flex-col items-center text-center gap-4">
            ...
            <div className="flex gap-3 w-full">
              <button onClick={onClose} className="flex-1 py-3 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-white text-sm font-semibold transition-all">Keep it</button>
              <button disabled={loading} onClick={handleDelete} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-red-600 hover:bg-red-500 disabled:opacity-60 text-white text-sm font-black transition-all shadow-lg shadow-red-600/20">
                {loading ? (
    ...
    >
      ...
      <div className="relative w-full h-44 overflow-hidden bg-gray-900 flex-shrink-0">
        {!imgError && facility.image ? (
          <img src={facility.image} alt={facility.facilityName} onError={() => setImgError(true)} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
        ) : (
      ...
      <div className="flex flex-col gap-3 p-4 flex-1">
        ...
        <div className="flex gap-2 pt-1">
          <button onClick={() => onEdit(facility)} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-blue-600/12 border border-blue-500/25 hover:bg-blue-600/25 hover:border-blue-500/50 text-blue-300 text-xs font-bold transition-all duration-200">
            <MdEdit size={14} /> Edit
          </button>
          <button onClick={() => onDelete(facility)} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-red-500/10 border border-red-500/20 hover:bg-red-500/20 hover:border-red-500/40 text-red-400 text-xs font-bold transition-all duration-200">
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center text-center py-24 px-4 gap-5">
      ...
      <Link href="/add-facilities" className="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm rounded-xl transition-all shadow-lg shadow-blue-600/25">
...
export default function ManageFacilitiesClient({ facilities: initial, ownerEmail }) {
  ...
  // Fixed handleSaved - সাথে সাথে আপডেট দেখাবে
  ...
  const handleDeleted = (id) => {
    setFacilities((prev) => prev.filter((f) => String(f._id) !== String(id)));
    setDeleting(null);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5 mb-10">
          <div>
            ...
            <h1 className="text-3xl sm:text-4xl font-black text-white tracking-tight leading-tight">Manage Facilities</h1>
            <p className="text-sm text-gray-400 mt-1.5">
              {facilities.length > 0 ? `You have ${facilities.length} listed facilit${facilities.length > 1 ? "ies" : "y"}.` : "No facilities listed yet."}
            </p>
          ...
          <Link href="/add-facilities" className="flex items-center gap-2 px-5 py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm rounded-xl transition-all shadow-lg shadow-blue-600/25 flex-shrink-0">
          ...
          <AnimatePresence mode="popLayout">
            <motion.div layout className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {facilities.map((facility, i) => (
                <motion.div key={facility._id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.92 }} transition={{ duration: 0.4, delay: i * 0.05 }}>
                  <ManageCard facility={facility} ownerEmail={ownerEmail} onEdit={setEditing} onDelete={setDeleting} />
                </motion.div>
              ))}
            </motion.div>
          </AnimatePresence>
      ...
      <AnimatePresence>
        {editing && (
          <EditModal key="edit" facility={editing} ownerEmail={ownerEmail} onClose={() => setEditing(null)} onSaved={handleSaved} />
        )}
      </AnimatePresence>

      <AnimatePresence>
        {deleting && (
          <DeleteModal key="delete" facility={deleting} ownerEmail={ownerEmail} onClose={() => setDeleting(null)} onDeleted={handleDeleted} />
        )}
      </AnimatePresence>
    </div>
  );
